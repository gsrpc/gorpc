...
const (
	tvnbits uint64 = 6
	tvrbits uint64 = 8
	tvnsize uint64 = 64  //1 << tvnbits
	tvrsize uint64 = 256 //1 << tvrbits

	tvnmask uint64 = 63  //tvnsize - 1
	tvrmask uint64 = 255 //tvrsize -1
)
...
//NewWheel tick is the time for a jiffies
...
func (w *Wheel) addTimerInternal(t *timer) {
	...
	if idx < tvrsize {
		...
		i = (expires >> (tvrbits + tvnbits)) & tvnmask
		tv = w.tv3
	} else if idx < (1 << (tvrbits + 3*tvnbits)) {
		i = (expires >> (tvrbits + 2*tvnbits)) & tvnmask
		tv = w.tv4
	} else if int64(idx) < 0 {
		i = w.jiffies & tvrmask
		tv = w.tv1
...
func (w *Wheel) getIndex(n int) int {
	return int((w.jiffies >> (tvrbits + uint64(n)*tvnbits)) & tvnmask)
}

func (w *Wheel) onTick() {
	w.Lock()

	index := int(w.jiffies & tvrmask)
